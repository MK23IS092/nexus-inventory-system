pipeline {
  ...
  stages {
    ...
    stage('Prepare Docker Engine') {
      steps {
        bat '''
        powershell -NoProfile -ExecutionPolicy Bypass -Command "$ErrorActionPreference = 'Stop'; $desktopExe = Join-Path $env:ProgramFiles 'Docker\Docker\Docker Desktop.exe'; $services = @('com.docker.service', 'Docker Desktop Service'); foreach ($serviceName in $services) { $svc = Get-Service -Name $serviceName -ErrorAction SilentlyContinue; if ($svc -and $svc.Status -ne 'Running') { try { Start-Service -Name $serviceName -ErrorAction Stop } catch { Write-Host \"Unable to start service $serviceName: $($_.Exception.Message)\" } } }; if (Test-Path $desktopExe) { $desktop = Get-Process -Name 'Docker Desktop' -ErrorAction SilentlyContinue; if (-not $desktop) { try { Start-Process -FilePath $desktopExe -ArgumentList '--minimized' } catch { Write-Host \"Unable to start Docker Desktop: $($_.Exception.Message)\" } } }; for ($i = 0; $i -lt 36; $i++) { docker info *> $null; if ($LASTEXITCODE -eq 0) { Write-Host 'Docker daemon is available.'; exit 0 }; Start-Sleep -Seconds 5 }; Write-Error 'Docker daemon is not available on this Jenkins agent. Start Docker Desktop or install the Docker service on the Windows agent.'; exit 1"
        '''
      }
    }
